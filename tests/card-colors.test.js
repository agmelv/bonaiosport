...
t(c.accentA === 'fc4c02', 'with an orange stripe', c.accentA);
t(c.b === img.NEUTRAL_HALF, 'the Raiders half is charcoal, not a shade of Dolphins aqua', c.b);
t(c.accentB === 'a5acaf', 'with a silver stripe', c.accentB);

console.log('--- the same with the real table, when it is present');
if (mia && lv) {
  c = img.cardColors({ ...dol, brand: mia }, { ...rai, brand: lv }, '0369a1');
  t(cc.isChromatic(c.a) && c.b === img.NEUTRAL_HALF && !!c.accentA, 'aqua half, charcoal half, a stripe', JSON.stringify(c));
}
...
t(c.accentA === '000e54', 'with a navy stripe', c.accentA);
...
t(c.accentA === null, 'and is not used for a stripe either', c.accentA);
...
t(c.accentA === null && c.accentB === null, 'no stripes', JSON.stringify(c));
